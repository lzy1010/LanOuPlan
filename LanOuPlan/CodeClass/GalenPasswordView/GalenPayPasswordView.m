...
-(void)setupInputView
{
    ...
    [self.cancelBtn setImage:[UIImage imageNamed:@"向右 copy"] forState:UIControlStateNormal];
...
-(void)lessPwdBtnView
{
    UIButton *noPWD=[[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.inPutView.frame)-140, CGRectGetMaxY(self.inPutView.frame)+15, 140, 60)];
    noPWD.titleLabel.font=[UIFont systemFontOfSize:14];
    
    [noPWD setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
...
{
    [self.lessPwdBtn setTitle:self.Server.api forState:UIControlStateNormal];
...
- (GalenPasswordServer *)Server{
    if (!_Server) {
        _Server = [GalenPasswordServer new];
    }
    return _Server;
}
